/*
ICode Library:Timer.h
This library implements the timer class Timer based on C++11 library chrono.
*/

#ifndef ICODE_TIMER__
#define ICODE_TIMER__
...
namespace ICode
{
	uint64_t now()
	{
		return std::chrono::duration_cast<std::chrono::milliseconds>(
		           std::chrono::system_clock::now() - std::chrono::time_point<std::chrono::system_clock>() ).count();
	}
				...
				public:
					...
					void run()
					{
						while ( true )
						{
							...
							if ( _flag.load() == false )
							{
								if ( _cv.wait_for( lck, std::chrono::milliseconds( sleep_time ), [this] ()->bool { return this->_flag.load(); } ) )
								{
