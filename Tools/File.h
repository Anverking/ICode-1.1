/*
ICode Library:File.h
This library provides some basic operations for files under Windows.
*/
...
namespace ICode
{
	class File
	{
		public:
			struct timer
			{
				...
				timer(int y=0,int m=0,int d=0,int h=0,int mi=0,int s=0):year(y),month(m),day(d),hour(h),minute(mi),second(s){}
			};	
			
		private:
			static const int BUFSIZE=4096,EXIST=0,READABLE=2,WRITABLE=4,EXECUTABLE=6;
	...
	{
		if(exists())
			return !_access(path.c_str(),WRITABLE);
	...
	{
		tm t=*localtime(&tt);
		return timer(t.tm_year+1900,t.tm_mon+1,t.tm_mday,t.tm_hour,t.tm_min,t.tm_sec);
	}
	...
	std::vector<File> File::listFile()
	{
		...
		if(isDirectory())
			for(auto& it:list())
				lstf.push_back(File(absolute_path+"\\"+it));
...
#else
#error "should include this library under Windows System"
#endif
