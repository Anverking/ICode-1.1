/*
ICode Library:optimize.h
This library calls some compilation optimizations 
and implements fast file reading and writing class optimized_fstream. 
Notice:the optimizations are not stable enough, please use it carefully.
*/ 

#ifndef ICODE_OPTIMIZE__
#define ICODE_OPTIMIZE__
...
{ 
	struct optf_control
	{
		...
		inline optf_control operator()(int Val)
		{
			return optf_control(ct,Val);
		}
	...
	*/
	class optimized_fstream
	{
		...
		public:
			optimized_fstream():p(in),pp(in),q(out),qq(out+IOSIZE),t(ch),b(1),K(6){}
			~optimized_fstream()
			{
	...
	#define indef(T) inline optimized_fstream& operator>>(T& x){\
	...
	#undef indef
			...
			inline optimized_fstream& operator>>(char& ch)
			{
	...
	#define outdef(_T) inline optimized_fstream& operator<<(_T x){\
