/*
ICode Library:ex_string.h
This library implements some extension algorithms for the STL string class.
*/

#ifndef ICODE_EX_STRING__
#define ICODE_EX_STRING__
	...
	namespace ex_string
	{
		...
		*/
		{
			...
			for(int i=0;i<n;++i)
				m=std::max(m,(int)str[i]);
			
			int *rank=new int[2*n+1],*a=new int[2*n+1],*b=new int[2*n+1];
		...
		*/
		{	
			std::string rev;		
			int n=str.size();
			int *s=new int[2*n+1],*sa=new int[2*n+1],*rank=new int[2*n+1],*h=new int[2*n+1];
			...
			public:
				Aho_Corasick_Automaton()
				{
					...
					for(int i=0;i<size;++i)
						c[i]=new int[CHARSET],
						memset(c[i],0,CHARSET*sizeof(int));
		...
		class Suffix_Automaton
		{
			...
			public:
				...
				Suffix_Automaton():last(1),cnt(1)
				{
					...
					for(int i=0;i<size;++i)
						ch[i]=new int[CHARSET],
						memset(ch[i],0,CHARSET*sizeof(int));
