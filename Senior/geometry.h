/*
ICode Library:geometry.h
This library provides some basic algorithms of computational geometry.
*/

#ifndef ICODE_GEOMETRY__
#define ICODE_GEOMETRY__ 
...
{	
	namespace geometry
	{
		...
		*/
		...
		/*
		Calculate the cross product p1¡Áp2.
		*/
		{
		...
		*/
		...
		/*
		Query the distance from the point to the straight line in three-dimensional space.
		*/
		{
		...
		*/
		...
		/*
		Determine whether the point P in two-dimensional plane is on the straight line L.
		*/
		{
		...
		/*
		Determine whether the point P in three-dimensional space is on the straight line L.
		*/
		{
			return ZERO( (p - L.p1) * (L.p2 - L.p1) );
		}
		...
		*/
		...
		/*
		Query the symmetric point of point P on two-dimensional plane with respect to line L.
		*/
		{
			Point result;
		...
		*/
		{
			return ( GreaterEqual( std::max(L1.p1.x, L1.p2.x), std::min(L2.p1.x, L2.p2.x) ) &&
		...
		/*
		The intersection of two two two-dimensional straight lines is calculated and the result is returned in parameter P.
		The return value indicates the position relationship between the two lines:
		COLINE=1:Collinear;
		PARALLEL=2:Parallel;
		...
		*/
		{
			...
			for (i = 1; i < n; i++)
			{
				if ((i + 1 < n) && Equal(points[i].angle, points[i+1].angle)) continue;
				if (result.size() >= 3)
				{
		...
		Result [LEFT] and result [RIGHT] save the left and right parts of the cut line,respectively.
		*/
		{
		...
		*/
		{
			return (GreaterThan(C.c.x - C.r, rect.xl)
			        &&  LessThan(C.c.x + C.r, rect.xh)
			        &&  GreaterThan(C.c.y - C.r, rect.yl)
			        &&  LessThan(C.c.y + C.r, rect.yh));
		}
		...
		*/
		{
			...
			if(LessThan(dis,c1.r+c2.r) && GreaterThan(dis,fabs(c1.r-c2.r))) return INTERSECT;
		...
		class scan_line
		{
			...
			public:
				...
				long long query_area()
				{
					...
					for(int i=1;i<=n<<1;++i)
					{
						long long pos1=std::lower_bound(dic+1,dic+tot+1,plane[i].y1)-dic;
						long long pos2=std::lower_bound(dic+1,dic+tot+1,plane[i].y2)-dic;
