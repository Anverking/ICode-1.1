/*
ICode Library:polynomial.h
This library implements some number theory functions and polynomial operations based on STL vector.
*/

#ifndef ICODE_POLYNOMIAL__
#define ICODE_POLYNOMIAL__
...
namespace ICode
{
	...
	namespace detail
	{
		...
		void __increase(int &x,int y)
		{
			(x+=y)>=ModForm&&(x-=ModForm);
		}
		void __decrease(int &x,int y)
		{
			(x-=y)<0&&(x+=ModForm);
		}
		int __increase(int x)
		{
			return x>=ModForm?x-ModForm:x;
		}
		int __decrease(int x)
		{
			return x<0?x+ModForm:x;
		}
		int __extend(int x)
		{
			int n=1;
	...
	namespace polynomial
	{
		...
		void NTT(polynomial &A,bool opt)
		{
			...
			for(int l=2;l<=n;l<<=1)
			{
				...
				if(opt)
					w=mathmetic::prime_inv(w,ModForm);
				...
				for(int i=0;i<n;i+=l)
					for(int j=0;j<m;++j)
					{
						...
						if(opt)
						{
							...
							A[i+j+m]=1LL*detail::__decrease(x-y)*I2%ModForm;
						}
						else
						{
		...
		polynomial inv (polynomial A)
		{
			...
			for(int l=2;l<=N;l<<=1)
			{
				...
				for(int i=0;i<L;++i)
					P[i]=1LL*Q[i]*(2-1LL*P[i]*Q[i]%ModForm+ModForm)%ModForm;
