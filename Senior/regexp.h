/*
ICode Library:regexp.h
This library implements the basic operation of regular expressions.
*/
...
#ifndef ICODE_REGEXP__
#define ICODE_REGEXP__ 
...
namespace ICode
{
	...
	namespace detail
	{
		...
		int __regexp_match_bracket_class (int c, const char *p, const char *ec)
		{
			...
			while (++p < ec)
			{
				...
				{
					p++;
					if (__regexp_match_class(c, uchar(*p)))
						return sig;
				}
				else if ((*(p+1) == '-') && (p+2 < ec))
				{
	...
	class regexp
	{
		...
		public:
			...
			regexp (char * s) : result (REGEXP_NOMATCH)
			{
				insert (s);
			}  
		...
		public:
			...
			*/
			unsigned int replace (const char*,GlobalReplaceCallback,const unsigned int max_count=0);
			/*
			Iterate with a supplied pattern, call function f for each match, maximum of max_count matches if max_count > 0,
			returns count of replacements.
			*/
			unsigned int replace (const char*,const char*,const unsigned int max_count=0);
			/*
	...
	private:
			...
			friend const char *classend (regexp *ms, const char *p)
			{
				switch (*p++)
				{
					case REGEXP_ESC:
						{
							if (*p == '\0')
								detail::__regexp_throw_error(ERR_MALFORMED_PATTERN_ENDS_WITH_ESCAPE);
							return p+1;
						}
					case '[':
						{
							...
							{
								if (*p == '\0')
									detail::__regexp_throw_error(ERR_MALFORMED_PATTERN_ENDS_WITH_RH_SQUARE_BRACKET);
								if (*(p++) == REGEXP_ESC && *p != '\0')
									p++;  
							}
							while (*p != ']');
			...
			friend const char *matchbalance (regexp *ms, const char *s,
			                                 const char *p)
			{
				if (*p == 0 || *(p+1) == 0)
			...
			friend const char *max_expand (regexp *ms, const char *s,
			                               const char *p, const char *ep)
			{
				int i = 0;  
				while ((s+i)<ms->src_end && detail::__regexp_single_match(uchar(*(s+i)), p, ep))
					i++;
			...
			friend const char *min_expand (regexp *ms, const char *s,
			                               const char *p, const char *ep)
			{
				for (;;)
				{
					...
					if (res != nullptr)
						return res;
					else if (s<ms->src_end && detail::__regexp_single_match(uchar(*s), p, ep))
						s++;  
					else return nullptr;
				}
			} 
			...
			friend const char *start_capture (regexp *ms, const char *s,
			                                  const char *p, int what)
			{
				const char *res;
				int level = ms->level;
				if (level >= MAXCAPTURES) detail::__regexp_throw_error(ERR_TOO_MANY_CAPTURES);
			...
			friend const char *end_capture (regexp *ms, const char *s,
			                                const char *p)
			{
								...
								case 'f':    
									{
										...
										p += 2;
										if (*p != '[')
											detail::__regexp_throw_error(ERR_MISSING_LH_SQUARE_BRACKET_AFTER_ESC_F);
										ep = classend(ms, p);  
										previous = (s == ms->src) ? '\0' : *(s-1);
										if (detail::__regexp_match_bracket_class(uchar(previous), p, ep-1) ||
										        !detail::__regexp_match_bracket_class(uchar(*s), p, ep-1)) return nullptr;
										p=ep;
										goto init;  
									}
								default:
					...
					dflt:    
						{
							...
							int m = s<ms->src_end && detail::__regexp_single_match(uchar(*s), p, ep);
							switch (*ep)
							{
								case '?':    
									{
										const char *res;
										if (m && ((res=str_match(ms, s+1, ep+1)) != nullptr))
											return res;
										p=ep+1;
										goto init;  
									}
	...
	char regexp::match (const char * pattern, unsigned int index)
	{
		...
		if (!src)
			detail::__regexp_throw_error (ERR_NO_TARGET_STRING);
	...
	unsigned int regexp::replace (const char * pattern, GlobalReplaceCallback f, const unsigned int max_count)
	{
		unsigned int count = 0;
	...
	unsigned int regexp::replace (const char * pattern, const char * replacement, const unsigned int max_count)
	{
		unsigned int count = 0;
