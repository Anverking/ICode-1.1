/*
ICode Library:BigInteger.h
This library implements high-precision integer class BigInteger
and it's basic operations.
*/

#ifndef ICODE_BIG_INTEGER__
#define ICODE_BIG_INTEGER__
...
namespace ICode
{
	...
	BigInteger operator+(const BigInteger& a,const BigInteger& b)
	{
		...
		int carry=0;
		int aa,bb;
		std::size_t lena=a.num.size(),lenb=b.num.size(),len=std::max(lena,lenb);
	...
	{		
		...
		std::size_t lena=faca.size(),lenb=facb.size(),len=std::min(lena,lenb);
	...
	BigInteger BigInteger::Exgcd(BigInteger a,BigInteger b,BigInteger& x,BigInteger& y) 
	{
		BigInteger d=a;
