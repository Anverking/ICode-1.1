/*
ICode Library:mathmetic.h
This library implements some mathematical (mainly number theory) algorithms,
as well as class Fraction.
*/

#ifndef ICODE_MATHMETIC__
#define ICODE_MATHMETIC__
...
namespace ICode
{
		...
		*/
		{
			...
			for(LL i=0;i<=m;b=multiply(b,a,p),++i)
				mp[b]=i;
	...
	#define INF (1e12)
		...
		/*
		Calculate the value of the polynomial coef(stores each coefficient) at the point x.
		*/
		{
			double e=1.0,s=0.0;
		...
		double __equation_find_root(const std::vector<double>&coef,int n,double l,double r,double eps=1e-8)
		{
			double signl,signr;
			
			if((signl=sign(interpolation(coef,l)))==0)
				return l;
			if((signr=sign(interpolation(coef,r)))==0)
				return r;
		...
		*/
		{
			...
			public:
				...
				{	
					if(den==0)
						throw("FRACTION:Divided by zero.");
				...
				int operator%(int x)
				{
					return num*mathmetic::inv(den,x)%x;
				}
