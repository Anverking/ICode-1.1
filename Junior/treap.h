/*
ICode Library:treap.h
This library implements the treap class,which supports interval addition,interval multiplication,and interval summation.
*/

#ifndef ICODE_TREAP__
#define ICODE_TREAP__
...
#define mod(x) (Mod==0?x:x%Mod)
	...
	*/
	...
	struct treap<__Tp,Mod>::Node
	{
		...
		Node(const __Tp& v=__Tp(),Node* l=nullptr,Node* r=nullptr,size_t ss=1,const __Tp& aff=__Tp(0),const __Tp& mff=__Tp(1)):
			val(v),sum(v),pri(rnd()),lc(l),rc(r),s(ss),addflag(aff),mulflag(mff) {}

		static size_t size(Node* ptr)
		{
			return ptr!=nullptr?ptr->s:0;
		}
	...
	template<typename __Tp,const __Tp Mod>
	void treap<__Tp,Mod>::ranksplit(Node* nroot,const size_t& k,Node*& ltree,Node*& rtree)
	{
		if(!nroot)
		{
			ltree=rtree=nullptr;
