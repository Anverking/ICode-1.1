/*
ICode Library:RMQ.h
This library implements the basic data structure of RMQ(Range Minimum/Maximum Query).
(static RMQ sparse table,dynamic RMQ segment tree,tree RMQ sparse table LCA).
*/
...
namespace ICode
{
	...
	template<typename __Tp,typename __Query=minimum_Q<__Tp>,const int MaxLog=33>
	class st_table
	{
		private:
			__Tp* st_cmp[MaxLog];
			...
			{	
				...
				for(int i=1;i<=pre_log[n]+1;++i)
					for(int j=1;j+(1<<i)-1<=n;++j)
						st_cmp[i][j]=_qry(st_cmp[i-1][j],st_cmp[i-1][j+(1<<i-1)]);
			} 
	...
	class segment_tree
	{
		private:
			struct Node
			{
				...
				Node(__Tp val=__Tp(),Node* lc=nullptr,Node* rc=nullptr):val(val),lc(lc),rc(rc){}
			...
			__Tp __query(Node* x,size_t L,size_t R,size_t l,size_t r)
			{
				...
				__Query _qry;
				__Tp lq=__query(x->lc,L,mid,l,mid),rq=__query(x->rc,mid+1,R,mid+1,r);
	...
	*/
	{
		private:
			...
			Node* __LCA(Node* x,Node* y)
			{
				...
				while(x->dep>y->dep)
					x=e[f[x->label][lg[x->dep-y->dep]-1]-1];
