/*
ICode Library:Trie.h
This library implements the basic operation of trie tree.
*/
	...
	*/ 
	class Trie
	{
		private:
			...
			void collect(std::string,std::string,Node*,std::vector<std::string>&) const;
	...
	template<const size_t CHARSET>
	typename Trie<CHARSET>::Node* Trie<CHARSET>::find(std::string key,Node* ptr,int d) const
	{
		if(ptr==nullptr)
			return nullptr;
	...
	template<const size_t CHARSET>
	void Trie<CHARSET>::collect(std::string key,Node* ptr,std::vector<std::string>& v) const
	{
		if(ptr==nullptr)
			return;
		...
		for(int i=0;i<CHARSET;++i)
			collect(key+(char)i,ptr->nxt[i],v);
	}
	...
	template<const size_t CHARSET>
	void Trie<CHARSET>::collect(std::string pre,std::string pat,Node* ptr,std::vector<std::string>& v) const
	{
		if(ptr==nullptr)
			return;
		...
		for(int i=0;i<CHARSET;++i)
			if(nxt=='.'||nxt==i)
				collect(pre+(char)i,pat,ptr->nxt[i],v);
	}
