/*
ICode Library:skip_list.h
This library encapsulates the skip_list class 
and implements some of the functions of the skip list.
*/ 

#ifndef ICODE_SKIP_LIST__
#define ICODE_SKIP_LIST__
...
namespace ICode
{	
	template<typename __Key,typename __Value,typename __Cmp=std::less<__Key>,uint32_t __Lvl=15>
	class skip_list
	{
		private:
			struct Node; 
		...
		public:
			...
			struct iterator:public std::iterator<std::input_iterator_tag,std::pair<__Key, __Value> >
			/*
			Class iterator.
			...
			*/
			{
				...
				public:
					...
					{
						return iter._real_node==_real_node;
					}
					...
					{
						return std::pair<__Key&,__Value&>(_real_node->key,_real_node->val);
					} 
	...
	template<typename __Key,typename __Value,typename __Cmp,uint32_t __Lvl>
	bool skip_list<__Key,__Value,__Cmp,__Lvl>::insert(const __Key key,const __Value val)
	{
	...
	template<typename __Key,typename __Value,typename __Cmp,uint32_t __Lvl>
	bool skip_list<__Key,__Value,__Cmp,__Lvl>::remove(const __Key key,__Value& tot)
	{
		Node* update[__Lvl],p=header;
		__Cmp _cmp;
	...
	template<typename __Key,typename __Value,typename __Cmp,uint32_t __Lvl>
	bool skip_list<__Key,__Value,__Cmp,__Lvl>::search(const __Key key,__Value& tot)
	{
		Node* p=header;
		__Cmp _cmp;
