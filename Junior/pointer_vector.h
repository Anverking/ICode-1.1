/*
ICode Library:pointer_vector.h
This library overrides the pointer-based non-fully encapsulated classes vector,stack and queue.
*/ 

#ifndef ICODE_POINTER_VECTOR__
#define ICODE_POINTER_VECTOR__
...
namespace ICode
{
	...
	class pointer_vector
	{
		private:
			virtual void shrink()
			{
				if(_cap<(_size<<2)||_cap<=MIN_items)
					return;
		...
		public:
			...
			pointer_vector(const pointer_vector& other)
			{
				*this=other;
			}
			...
			__Tp& operator[](const uint32_t rank)
			{
				return items[rank];
			}
...
#define real_size (_size-ex_sz)
	...
	class pointer_stack:private pointer_vector<__Tp>
	{
		...
		public:
			pointer_stack(){}
			pointer_stack(const pointer_stack& other)
			{
				*this=other;
			}
	...
	class pointer_queue:private pointer_vector<__Tp>
	{
		...
		private:
			...
			void shrink()
			{
				if(_cap<(real_size<<2)||_cap<=MIN_items)
					return;
				
				__Tp* olditems=items;
				items=new __Tp[_cap>>=1];
				
				for(uint32_t i=ex_sz;i<_size;++i)
					items[i-ex_sz]=olditems[i];
		...
		public:
			...
			pointer_queue(const pointer_queue& other):ex_sz(0)
			{
				*this=other;
			}
