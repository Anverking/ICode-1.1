/*
ICode Library:peak.h
This library realizes the search of data peaks.
*/
	...
	*/
	class peak
	{
		private:
			...
			void find_peaks()
			{
				...
				for(size_t i=1;i<siz-1;++i)
					if((_cmp(m_freq[i-1],m_freq[i])||m_freq[i]==m_freq[i-1])&&
			...
			void remove_below(__Tp minT,__Tp minH,size_t minW)
			{
				...
				for(size_t i=0;i<siz;++i)
				{
					auto idx=m_ipks[i];
					__Cmp _cmp;
					auto v=_cmp(m_freq[idx-1],m_freq[idx+1])?m_freq[idx+1]:m_freq[idx-1];
					
					if(m_freq[idx]-v>minT)
				...
				std::sort(pks.begin(),pks.end(),[](const std::pair<__Tp,size_t>& x,const std::pair<__Tp,size_t>& y)
				{
					__Cmp _cmp;
				...
				for(size_t i=0;i<siz;++i)
					if(idel[i]==0)
					{
						for(size_t j=0;j<idel.size();++j)
							idel[j]=idel[j]||(pks[j].second+minW>=pks[i].second)&&(pks[j].second<=pks[i].second+minW);
		...
		public:
			...
			peak(int dis=0,__Tp ht=__Tp(),__Tp thre=__Tp()):m_dis(dis),m_ht(ht),m_thre(thre){}
