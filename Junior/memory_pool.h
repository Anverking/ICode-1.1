/*
ICode Library:memory_pool.h
This library contains two classes:
the memory_pool class is a dynamic memory pool for requesting memory;
the memory_allocator class is used for STL memory requests.
*/ 

#ifndef ICODE_MEMORY_POOL__
#define ICODE_MEMORY_POOL__
...
namespace ICode
{
	...
	class memory_pool
	{
		private:
			...
			static_assert(block_size>=2*sizeof(slot_type),"MEMORY_POOL:BlockSize too small.");
			...
			*/
			{
				...
				else
				{
					...
					{
						data_pointer new_block=reinterpret_cast<data_pointer>(operator new(block_size));
						...
						size_t body_padding=(alignof(slot_type)-result)%alignof(slot_type);
						current_slot=reinterpret_cast<slot_pointer>(body+body_padding);
						last_slot=reinterpret_cast<slot_pointer>(new_block+block_size-sizeof(slot_type));
					}
