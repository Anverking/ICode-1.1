/*
ICode Library:FFT.h
This library implements the basic operation of Fast Fourier Transform (FFT).
*/
...
namespace ICode
{
	namespace detail
	{
		...
		static void __FFT_Rearrange(const complex<__Tp>* const Input,const complex<__Tp>* Output,const unsigned int N)
		{
			unsigned int tar=0;
		...
		template<typename __Tp>
		static void __FFT_Perform(const complex<__Tp>* Data,const unsigned int N,const bool opt)
		{
			...
			for(unsigned int i=1;i<N;i<<=1)
			{
				...
				const complex<__Tp> Mul(static_cast<__Tp>(-2.0*Sine*Sine),static_cast<__Tp>(sin(delta)));
	...
	{	
		template<typename __Tp>
		static bool DFT(const complex<__Tp>* const Input,const complex<__Tp>* Output,const unsigned int N)
		/*
		...
		*/
		...
		template<typename __Tp>
		static bool IDFT(const complex<__Tp>* const Input,const complex<__Tp>* Output,const unsigned int N,const bool scale=true)
		/*
		...
		*/
		{
			...
			if(scale)
				detail::__FFT_Scale(Output,N);
			
			return true;
		}
		
		template<typename __Tp>
		static bool IDFT(const complex<__Tp>* Data,const unsigned int N,const bool scale=true)
		/*
