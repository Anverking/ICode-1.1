/*
ICode Library:sequence.h
This library implements some algorithms for arrays.
*/
#ifndef ICODE_SEQUENCE__
#define ICODE_SEQUENCE__
...
namespace ICode
{
	...
	{		
		...
		void __select_kth(__Tp* a,const int l,const int r,const int k)
		{
			...
			if(l<=j&&k<=j-l+1)
				__select_kth(a,l,j,k);
			if(i<=r&&k>=i-l+1)
				__select_kth(a,i,r,k-(i-l));
		}
	}
	namespace sequence
	{
		...
		*/
		{
			...
			for(int i=0;i<=2*n1;++i)
				f[i]=new int[2*n2+1],
				memset(f[i],0,(2*n2+1)*sizeof(int));
		...
		class array_hash
		{
			...
			public:
				...
				/*
				Query the ranking of a given full permutation from 1 to N in all 1 to n full permutations,
				The array must be stored from subscript 1.
				(Cantor Expansion)
				*/
				{
