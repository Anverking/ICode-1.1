/*
ICode Library:dancing_links.h
This library implements the dancing links-X (DLX) algorithm for solving Exact Cover Problem.
*/

#ifndef ICODE_DANCING_LINKS__
#define ICODE_DANCING_LINKS__
	...
	class dancing_links
	{
		private:
			struct Node
			{
				...
				Node(int r=0,int c=0,Node* U=nullptr,Node* D=nullptr,Node* L=nullptr,Node* R=nullptr):r(r),c(c),U(U),D(D),L(L),R(R){}
			};
			Node* node;
			Node* row;
			Node* col;
			...
			inline void reinstate_row(Node *ptr)
			{
			    ptr->L->R=ptr->R->L=ptr;
			}
			
			inline void reinstate_column(Node *ptr)
			{
			    ptr->U->D=ptr->D->U=ptr;
			}
