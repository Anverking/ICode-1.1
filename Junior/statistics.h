/*
ICode Library:statistics.h
This library implements some basic statistical functions.
Author:@Anverking
*/

#ifndef ICODE_STATISTICS__
#define ICODE_STATISTICS__
...
{			
	namespace statistics
	{
		...
		*/
		...
		__Tp normal_distribution(__Tp x,__Tp mu=(__Tp)(NULL),__Tp sigma=(__Tp)(1.0))
		{
			assert(sigma>(__Tp)(NULL));
			return (((__Tp)1.0)/((__Tp)(sqrt(2.0*acos(-1.0)))*sigma))*exp((-(x-mu)*(x-mu))/((__Tp)2.0*sigma*sigma));
		}
		...
		__Tp gamma_distribution(__Tp x,__Tp alpha,__Tp beta)
		{
			...
			return x>(__Tp)(NULL)?(((__Tp)1.0)/(pow(beta,alpha)*tgamma(alpha)))*pow(x,alpha-(__Tp)(1.0))*exp(-x/beta):(__Tp)(NULL);
		}
		...
		__Tp exponential_distribution(__Tp x,__Tp theta)
		{
			...
			return x>(__Tp)(NULL)?(((__Tp)1.0)/theta)*exp(-x/theta):(__Tp)(NULL); 
		} 
		...
		{
			assert(n>=(__Tp)(1.0));
			return x>(__Tp)(NULL)?(((__Tp)1.0)/(pow((__Tp)(2.0),n/(__Tp)(2.0))*tgamma(n/(__Tp)(2.0)))*pow(x,n/(__Tp)(2.0)-(__Tp)(1.0))*exp(-x/(__Tp)(2.0))):(__Tp)(NULL);	
		} 
		...
		__Tp weibull_distribution(__Tp x,__Tp eta,__Tp beta)
		{
			...
			return x>(__Tp)(NULL)?(beta/eta)*pow(x/eta,beta-(__Tp)(1.0))*exp(-pow(x/eta,beta)):(__Tp)(NULL);
		}
		...
		{
			assert(sigma>(__Tp)(NULL));
			return x>(__Tp)(NULL)?(x/(sigma*sigma))*exp(-((x*x)/((__Tp)(2.0)*sigma*sigma))):(__Tp)(NULL);
		}
		...
		{
			assert(alpha>(__Tp)(NULL)&&beta>(__Tp)(NULL));
			return (__Tp)(NULL)<x&&x<(__Tp)(1.0)?(tgamma(alpha+beta)/(tgamma(alpha)*tgamma(beta)))*pow(x,alpha-(__Tp)(1.0))*pow((__Tp)(1.0)-x,beta-(__Tp)(1.0)):(__Tp)(NULL);
		}
		...
		{
			assert(sigma>(__Tp)(NULL));
			return x>(__Tp)(NULL)?(((__Tp)(1.0))/((__Tp)(sqrt(2.0*acos(-1.0)))))*exp((-(log(x)-mu)*(log(x)-mu))/((__Tp)(2.0)*sigma*sigma)):(__Tp)(NULL);
		}
		...
		__Tp cauchy_distribution(__Tp x,__Tp a,__Tp lambda)
		{
			...
			return ((__Tp)(1.0/acos(-1.0)))*(lambda/(lambda*lambda+(x-a)*(x-a)));
		}
		...
		{
			assert(n>=(__Tp)(1.0));
			return ((tgamma((n+(__Tp)(1.0))/(__Tp)(2.0)))/(sqrt(n*x)*tgamma(n/((__Tp)(2.0)))))*pow((__Tp)(1.0)+x*x/n,-((n+(__Tp)(1.0))/(__Tp)(2.0)));
		}
		...
		{
			return x>(__Tp)(NULL)?
			(tgamma((n1+n2)/((__Tp)(2.0)))/(tgamma(n1/(__Tp)(2.0))*tgamma(n2/(__Tp)(2.0))))*(n1/n2)*pow(n1/n2*x,n1/(__Tp)(2.0)-(__Tp)(1.0))*
			pow((__Tp)(1.0)+n1/n2*x,-((n1+n2)/(__Tp)(2.0))):(__Tp)(NULL);
		}
		...
		*/
		{
			...
			public:
				...
				*/
				{
					...
					variance_record(__Tp ss=(__Tp)(NULL),__Tp va=(__Tp)(NULL),__Tp me=(__Tp)(NULL),__Tp F=(__Tp)(NULL)):
				...
				variance_analysis(std::vector<std::vector<__Tp> >_X)
				{
					build(_X);
				}
				...
				*/
				{
					return variance_record(S_A,s-1,S_A/(s-(__Tp)1.0),S_A/(s-(__Tp)1.0)/(S_E/((__Tp)n-(__Tp)s)));
				}
		...
		class linear_regression
		{
			...
			public:
				...
				linear_regression(std::vector<__Tp>x,std::vector<__Tp>y,bool res=nullptr)
				{
					if(res)
						res=build(x,y);
					build(x,y);
				...
				*/
				{
					...
					for(int i=0;i<n;++i)
					{
						...
						_t=
						t*sigma*sqrt(((__Tp)(1.0))/n+sqr(x_0-r_x)/S_xx);
				...
				*/
				{
					S_xx=S_xy=S_yy=sigma_sqr=sigma=t=Q_e=alpha=beta=r=r_x=r_y=(__Tp)(NULL);
					n=0;
					H=false;				
				}
				...
				*/
				{
					__Tp _t=t*sigma*sqrt(((__Tp)(1.0))/n+sqr(x-r_x)/S_xx),y_0=alpha+beta*x;
