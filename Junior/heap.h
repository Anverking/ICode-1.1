/*
ICode Library:heap.h
This library implements the basic operation of heap.
*/
		...
		protected:
			...
			void adjustdown(size_t node)
			{
				...
				while(true)
				{
					...
					if(l<siz&&cmp(_data[l],_data[node]))
						nxt=l;
					if(r<siz&&cmp(_data[r],_data[nxt]))
						nxt=r;
			...
			*/
			{
				...
				while(true)
				{
					...
					{
						_data.erase(it);
						fix_node=std::min(fix_node,(size_t)std::distance(_data.begin(),it));
					}
					else
						break;
				}
				...
				for(size_t node=fix_node;node<max_node;++node)
					adjustup(node);
			}
	};
}
