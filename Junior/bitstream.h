/*
ICode Library:bitstream.h
This library implements the class bitstream that can read bits from files and write bits into files.
*/

#ifndef ICODE_BITSTREAM__
#define ICODE_BITSTREAM__
	...
	class bitstream
	{
		...
		public:
			...
			*/
			{
				...
				while(_write_bit_offset>0)
				{
					...
					if(_write_buff_size>=buff_size)
						if(write_internal()!=_write_buff_size)
							_write_buff_size=0;
				}
			...
			*/
			{
				...
				if(_read_offset>=_read_buff_size)
					_read_buff_size=0;
			...
			*/
			{
				assert(bit==0||bit==1);
				bs._writebuf[bs._write_buff_size]=(bs._writebuf[bs._write_buff_size]<<1)|bit;
