/*
ICode Library:pratition.h
This library implements the class partition, which is a disjoint set data structure with path compression..
*/
#ifndef ICODE_DISJOINT_SET__
#define ICODE_DISJOINT_SET__
...
namespace ICode
{
	...
	template<typename __Tp>
	inline disjoint_set<__Tp>::disjoint_set(const __Tp siz):_parents(static_cast<size_t>(siz)),_ranks(static_cast<size_t>(siz)),_count(siz)
	{
		for(__Tp i=0;i<siz;++i)
			_parents[static_cast<size_t>(i)]=i;
	}
	
	template<typename __Tp>
	inline void disjoint_set<__Tp>::assign(const __Tp siz)
	{
	...
	template<typename __Tp>
	inline __Tp disjoint_set<__Tp>::size() const
	{
		return static_cast<__Tp>(_parents.size());
	}
	
	template<typename __Tp>
	inline __Tp disjoint_set<__Tp>::count() const
	{
		return _count;
	}
	
	template<typename __Tp>
	inline __Tp disjoint_set<__Tp>::const_find(const __Tp element) const
	{
		__Tp root=element;
	...
	template<typename __Tp>
	inline __Tp disjoint_set<__Tp>::find(__Tp element)
	{
		if(_parents[element]!=element)
			return _parents[static_cast<size_t>(element)]=find(_parents[static_cast<size_t>(element)]);
		return element;
	}
	
	template<typename __Tp>
	inline void disjoint_set<__Tp>::merge(__Tp element1,__Tp element2)
	{
		element1=find(element1);
		element2=find(element2);
		
		if(_ranks[static_cast<size_t>(element1)]<_ranks[static_cast<size_t>(element2)])
		{
			_parents[static_cast<size_t>(element1)]=element2;
			--_count;
		}
		else if(_ranks[static_cast<size_t>(element1)]>_ranks[static_cast<size_t>(element2)])
		{
	...
	template<typename __Tp>
	inline void disjoint_set<__Tp>::insert(const __Tp number)
	{
	...
	template<typename __Tp>
	template<typename __Iter>
	inline void disjoint_set<__Tp>::representatives(__Iter it) const
	{
	...
	template<typename __Tp>
	inline void disjoint_set<__Tp>::representative_labeling(std::map<__Tp,__Tp>& out) const
	{
		out.clear();
		std::vector<__Tp> r(static_cast<size_t>(count()));
		representatives(r.begin());
		
		for(__Tp i=0;i<count();++i)
			out[r[static_cast<size_t>(i)]]=i;
	}
	
	template<typename __Tp>
	template<typename __Iter>
	inline void disjoint_set<__Tp>::labeling(__Iter out) const
	{
