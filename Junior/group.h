/*
ICode Library:group.h
This library implements a class that can batch elements.
*/

#ifndef ICODE_GROUP__
#define ICODE_GROUP__
...
namespace ICode
{
	...
	class group
	{
		private:
			...
			void pushup()
			{
				for(typename std::vector<__Tp>::iterator iter=_base.begin();iter!=_base.end();++iter)
		...
		public:
			...
			group(const std::vector<__Tp>& vec):_base(vec),_addtag(__Tp(NULL)),_multag(__Tp(1)){}
			group(const group& other):_base(other._base),_addtag(other._addtag),_multag(other._multag){}
