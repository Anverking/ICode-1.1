/*
ICode Library:linear_algebra.h
This library implements some basic operations of the matrix.
*/

#ifndef ICODE_LINEAR_ALGEBRA__
#define ICODE_LINEAR_ALGEBRA__
...
#define rcm_t const matrix_t&
		...
		public:
			explicit matrix(size_t h=2,size_t w=2,__Tp init=zero):height(h),width(w)
			{
				vec.resize(height);
			...
			matrix(std::initializer_list<std::initializer_list<__Tp> >ini):height(ini.size())
			{
				vec.resize(height);
			...
			friend bool operator==(rcm_t lhs,rcm_t rhs)
			{
			...
			friend std::istream& operator>>(std::istream& is,matrix_t& mat)
			{
				...
				if(!is)
					mat=matrix_t(mat.height,mat.width);
...
#undef rcm_t
	...
	template<typename __Tp> 
	bool Gauss_elimination(matrix<__Tp>a,std::vector<__Tp>&b,const __Tp eps=1e-8)
	/*
	Gauss elimination.
	...
	*/
	{
		...
		if(a.size().first!=a.size().second-1)
			return false;
	...
	class linear_programming
	{
		private:
			...
			bool Pivot(std::pair<size_t,size_t>& p,const __Tp eps=1e-8,const __Tp INF=INT_MAX)
			{
				int x=0,y=0;
				__Tp cmax=-INF;
			...
			*/
			{
				...
				{
					if(!Pivot(t,eps))
						return std::make_pair(-mat[0][cn-1],P);
