/*
ICode Library:link_cut_tree.h
This library provides the link_cut_tree class and support some basic operations of the dynamic tree.
*/

#ifndef ICODE_LINK_CUT_TREE__
#define ICODE_LINK_CUT_TREE__
...
{	
	namespace detail
	{
		...
		struct LCT_splay<Value_type,Functor>::Node
		{
			...
			Node(const Value_type& v):val(v),sum(v),ftr(nullptr),lc(ch[0]),rc(ch[1]),rev(false)
			{
				ch[0]=ch[1]=nullptr;
			}
		...
		void LCT_splay<Value_type,Functor>::Node::maintain()
		{
		    ...
		    Functor _func;
		    if(lc!=nullptr)
				sum=_func(lc->sum,sum);
		    if(rc!=nullptr)
				sum=_func(sum,rc->sum);
		}
		...
		template<typename Value_type,typename Functor>
		typename LCT_splay<Value_type,Functor>::Node* LCT_splay<Value_type,Functor>::__new_node(const Value_type& v)
		{
			return new Node(v);
		}
	}
	...
	*/
	{
		...
		public:
			link_cut_tree(){}
			link_cut_tree(const link_cut_tree& other)
			{
				*this=other;
			}
	...
	template<typename Value_type,typename Functor>
	typename link_cut_tree<Value_type,Functor>::Node* link_cut_tree<Value_type,Functor>::find_root(Node* ptr)
	{
		access(ptr);
		ptr->splay();
	...
	template<typename Value_type,typename Functor>
	typename link_cut_tree<Value_type,Functor>::iterator link_cut_tree<Value_type,Functor>::insert(const Value_type& v)
	{
	...
	template<typename Value_type,typename Functor>
	typename link_cut_tree<Value_type,Functor>::iterator link_cut_tree<Value_type,Functor>::LCA(const iterator& siter,const iterator& eiter)
	{
		Node *sptr=siter._real_node,*eptr=eiter._real_node;
		access(sptr);
