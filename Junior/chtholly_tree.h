/*
ICode Library:chtholly_tree.h
This library implements the basic operation of Chtholly Tree(also known as Old Driver Tree).
Chtholly Tree is an interval data structure that can spread O(logn) complexity equally under random data.
*/

#ifndef ICODE_CHTHOLLY_TREE__
#define ICODE_CHTHOLLY_TREE__
...
namespace ICode
{
	...
	template<typename __Tp,const __Tp Mod=0,typename __Base=std::set<detail::chtholly_node<__Tp> > >
	class chtholly_tree:public __Base
	{
		...
		private:
			inline __Tp mod(__Tp x)
			{
				return Mod==0?x:x%Mod;
			}
			
			iterator split(int pos)
			{
				iterator itl=this->lower_bound(Node(pos,-1,__Tp()));
				if(itl!=this->end()&&itl->l==pos)
						return itl;
				--itl;
