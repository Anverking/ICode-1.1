/*
ICode Library:sorted_queue.h
This library provides a queue-based sorted_queue based on deque 
that can be automatically sorted. 
*/

#ifndef ICODE_SORTED_QUEUE__
#define ICODE_SORTED_QUEUE__
...
namespace ICode
{
	...
	class sorted_queue
	{
		...
		public:
			sorted_queue(){}
			sorted_queue(const sorted_queue& other)
			{
				*this=other;
			}
			...
			reference operator[](size_type __Pos_)
			{
			...
			{
				return __Main_Q.empty();
			}
			
			inline size_type size()const
			{
				return __Main_Q.size();
			}
			
			reference front()
			{
				return __Main_Q.front();
			}
			...
			{
				__Main_Q.insert(std::lower_bound(__Main_Q.begin(),__Main_Q.end(),__Val_),__Val_);
			}
			...
			iterator erase(const_iterator __Pos_)
			{
				return __Main_Q.erase(__Pos_);
			}
